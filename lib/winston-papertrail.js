...
Papertrail.prototype.sendMessage = function (hostname, program, level, message, callback) {
    ...
    if (this.stream && this.stream.writable) {
        ...
        //Set up an event to be triggered when the buffer is emptied on connection,
        //so the callback can still be executed when a buffer is streamed.
        self.once('bufferEmptied', callback);
    }
};
